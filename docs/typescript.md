...
**92** rules are included from the default config. For brevity, only the **29** additional rules are listed in this document.

Refer to the extended config's docs:

- [`@code-pushup` rules](./index.md#📏-rules-92)
...
| [![@typescript-eslint](./icons/typescript.png)](https://typescript-eslint.io/) | [require-await](https://typescript-eslint.io/rules/require-await)<br>Disallow async functions which have no `await` expression |  |  | 🧪⚠️ |
